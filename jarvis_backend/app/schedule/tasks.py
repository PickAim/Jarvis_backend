...
def __cache_update():
    ...
    WORKER_TO_STATUS[CacheWorker.identifier] = True
...
def __load_niches(load_skip: int):
    ...
    WORKER_TO_STATUS[LoadWorker.identifier] = True
...
if background_config.cache_enabled:
    SIMPLE_TASKS.append(SimpleTask(__cache_update, IntervalTrigger(days=1), identifier=CacheWorker.identifier))

if background_config.load_enabled:
    SIMPLE_TASKS.append(SimpleTask(__load_niches, IntervalTrigger(days=1),
                                   identifier=LoadWorker.identifier, args=[background_config.load_skip]))
