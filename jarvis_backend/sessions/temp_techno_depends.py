...
def __fetch_or_load_niche(session, marketplace_id: int, category_id: int, user_id: int) -> tuple[Niche, int]:
    ...
    if found_info is not None:
        ...
        jorm_changer = JDBClassesFactory.create_jorm_changer(session, marketplace_id, user_id=user_id)
...
def __init_tech_no_prom_niche(session):
    ...
    if len(id_to_product) != 0:
        return
    niche, niche_id = __fetch_or_load_niche(session, marketplace_id, 30, user_id)
